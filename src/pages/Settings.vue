<template>
    <q-page class="q-pa-md q-gutter-y-sm">
        ...
        <q-page-sticky position="bottom-right" :offset="[30, 30]">
            <q-btn icon="edit" class="row justify-end" push round padding="md" @click="newAlias = true" color="primary"></q-btn>
        </q-page-sticky>
        ...
        <q-dialog v-model="newAlias" position="right">
            <q-card class="alias-dialog flex flex-center q-pa-md" :dark="darkMode">
                ...
                <q-select class="alias-select" :dark="darkMode" filled v-model="currAliasee" :options="store.getBettors().map(p=> p.name)" label="Bettor"></q-select>
                <q-input v-if="currAliasee" class="alias-text" :dark="darkMode" v-model="text" label="Alias" hint="seperate multiple aliases with a comma (,)" :dense="dense">
                    <template v-slot:after>
                        <q-btn round dense flat icon="add" @click="setAlias(currAliasee, text)"/>
                    </template>
                </q-input>
            </q-card>
        </q-dialog>
...
export default defineComponent({
    ...
    setup() {
        ...
        return {
            ...
            getFilteredBettors() {
                return store.getBettors().filter(bettor => bettor.alias != bettor.name)
            },
